function [Tracks]=fill_gaps3D(Tracks)
    ...
    if (Tracks{lauf}(end,1)-Tracks{lauf}(1,1)) > length(Tracks{lauf})-1
        ...
        for  lauf2=2:length(Tracks{lauf})
            if (Tracks{lauf}(lauf2,1)-Tracks{lauf}(lauf2-1,1)) > 1
                ...
                blub4=interp1([Tracks{lauf}(lauf2-1,1),Tracks{lauf}(lauf2,1)],[Tracks{lauf}(lauf2-1,4),Tracks{lauf}(lauf2,4)],Tracks{lauf}(lauf2-1,1):1:Tracks{lauf}(lauf2,1));                
                interim=[interim(1:find(interim(:,1)==Tracks{lauf}(lauf2-1,1)),:);[Tracks{lauf}(lauf2-1,1)+1:1:Tracks{lauf}(lauf2,1)-1;blub2(2:end-1);blub3(2:end-1);blub4(2:end-1)]';interim(find(interim(:,1)==Tracks{lauf}(lauf2,1)):end,:)];%(Tracks{lauf}(lauf2,1)-Tracks{lauf}(1,1):Tracks{lauf}(end,1)-Tracks{lauf}(1,1)+Tracks{lauf}(lauf2,1)-Tracks{lauf}(lauf2-1,1)-1,:)=interim(Tracks{lauf}(lauf2-1,1)-Tracks{lauf}(1,1):Tracks{lauf}(end,1)-Tracks{lauf}(1,1)-1,:);
