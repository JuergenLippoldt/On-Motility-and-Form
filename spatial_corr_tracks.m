function [spa_vel_corr,spa_or_corr]=spatial_corr_tracks(Tracks,FrameTracks,time_windows,selection,mueh_per_pixel)
...
for f=time_windows(1):time_windows(2)
    ...
    xor_all=[xor_all, arrayfun(@(NT) (Tracks{NT}(Tracks{NT}(:,1)==f-1,29)/((Tracks{NT}(Tracks{NT}(:,1)==f-1,29))^2+(Tracks{NT}(Tracks{NT}(:,1)==f-1,30))^2)^.5),selection{f})];
	yor_all=[yor_all, arrayfun(@(NT) (Tracks{NT}(Tracks{NT}(:,1)==f-1,30)/((Tracks{NT}(Tracks{NT}(:,1)==f-1,29))^2+(Tracks{NT}(Tracks{NT}(:,1)==f-1,30))^2)^.5),selection{f})];   
end
...
for f=time_windows(1):time_windows(2)
    ...
    FrameTrack_with_vel=FrameTrack_with_vel(~logical(sum(isnan(cell2mat(arrayfun(@(tiq) Tracks{tiq}(Tracks{tiq}(:,1)==f-1,29:30)',FrameTrack_with_vel,'UniformOutput',false))))));
 	distance_vec_all=ipdm(mueh_per_pixel*cell2mat(arrayfun(@(tiq) Tracks{tiq}(Tracks{tiq}(:,1)==f-1,2:3)',selection_with_vel,'UniformOutput',false))',mueh_per_pixel*cell2mat(arrayfun(@(tiq) Tracks{tiq}(Tracks{tiq}(:,1)==f-1,2:3)',FrameTrack_with_vel,'UniformOutput',false))','Subset','Maximum', 'Limit',100,'Result','Structure');
